...
Create:
- <feature-root>/p1-switch-existing-branch/02-model.md
...
Model focus:
- model the first bounded switch-existing slice after the completed executor-side `P1 / Minor Change` branch-state hardening step and the completed first bounded create-and-switch-from-main slice
- make explicit the key concepts, switch-existing states, data structures, state transitions, operator-intent semantics, and stop behavior needed for the first slice
...
- keep the completed branch-state hardening baseline and completed first branch-mutation baseline mandatory before any bounded switch-existing execution is considered
- keep generalized Git workflow orchestration explicitly out of scope for this model
- keep the still-unresolved path-scope question explicit where it is not yet safely decided
...
- Do not resolve the open path-scope question by assumption; model it as an explicit decision point where necessary.
