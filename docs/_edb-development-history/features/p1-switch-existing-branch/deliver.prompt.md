...
Deliver focus:
- lock the first bounded implementation contract for switching to an already-existing branch after the completed executor-side `P1 / Minor Change` branch-state hardening step and the completed first bounded create-and-switch-from-main slice
- reflect the chosen analysis outcome:
  - keep the first switch-existing slice on the existing `P1` executor continuity path outside the Entry
...
- preserve the completed `P1` contract-emission baseline, the completed first executor-side `P1` minimal write baseline, the completed branch-state hardening baseline, the completed create-and-switch-from-main baseline, explicit stop behavior, and the explicit exclusion of generalized Git workflow orchestration
- make the first switch-existing implementation slice explicit enough to authorize later code work without broadening into non-`main` voluntary switching, create-and-switch baseline redesign, target-resolution redesign, or wider Git workflow automation
...
- Do not broaden scope into non-`main` voluntary switch-existing behavior, create-and-switch-from-main redesign, target-resolution redesign, template-aware generation, `P2`/`P3`/`P4`/`P5` behavior, PR-helper execution, UI/Vite work, or generalized Git workflow orchestration.
