...
Create:
- <feature-root>/p1-non-main-switch-existing-branch/03-analyze.md
...
Analyze focus:
- compare the smallest credible implementation shapes for the first bounded safe non-`main` switch-existing slice after the completed executor-side `P1 / Minor Change` branch-state hardening step, the completed first bounded create-and-switch-from-main slice, the completed first bounded switch-existing-from-main slice, and the completed first bounded non-`main` branch-choice slice
- explicitly evaluate the currently open question:
  - what exact bounded decision/result shape should expose the three safe non-`main` options without reshaping the broader branch model beyond this slice
- explicitly compare at least these bounded options:
  - preserve the existing safe non-`main` continuation path and the completed additional-new-branch path as they are, and add one separate bounded switch-existing path
  - introduce one new explicit three-option branch-choice envelope immediately for the safe non-`main` context
- preserve the accepted placement rule, the completed hardening baseline, the completed create-and-switch-from-main baseline, the completed blocked-`main` switch-existing baseline, the completed safe non-`main` additional-new-branch baseline, explicit stop behavior, and the explicit exclusion of generalized Git workflow orchestration
- recommend the thinnest implementation shape that adds bounded safe non-`main` switch-existing behavior on the existing `P1` executor continuity path without reopening blocked-`main` redesign, safe non-`main` create-and-switch redesign, target-resolution redesign, or wider Git workflow automation
...
- Do not resolve the open decision/result-shape question by hand-waving; compare real bounded options and document the recommendation explicitly.
