...
Create:
- <feature-root>/p1-branch-create-or-switch/03-analyze.md
...
- Existing behavior/system contracts to consider:
  ...
  - `docs/_edb-development-history/features/p1-branch-create-or-switch/questions.md`
...
Analyze focus:
- compare the smallest credible implementation shapes for the first bounded branch-mutation slice after the completed executor-side `P1 / Minor Change` branch-state hardening step
- explicitly evaluate the currently open questions:
  - what exact branch-target input should the first bounded mutation slice consume after `request_branch_change`
  - whether the first mutation slice should support both branch creation and switching, or only one narrower path first
- preserve the accepted placement rule, the completed hardening baseline, explicit stop behavior, and the explicit exclusion of generalized Git workflow orchestration
- recommend the thinnest implementation shape that adds bounded branch mutation on the existing `P1` executor continuity path without reopening target-resolution redesign or wider Git workflow automation
...
- Do not broaden scope into target-resolution redesign, template-aware generation, `P2`/`P3`/`P4`/`P5` behavior, PR-helper execution, UI/Vite work, or generalized Git workflow orchestration.
- Do not resolve the open branch-target-input or first-slice create-vs-switch questions by hand-waving; compare real bounded options and document the recommendation explicitly.
