...
Create:
- <feature-root>/p1-branch-create-or-switch/02-model.md
...
Model focus:
- model the first bounded branch-mutation slice after the completed executor-side `P1 / Minor Change` branch-state hardening step
- make explicit the key concepts, branch-mutation states, data structures, state transitions, operator-intent semantics, and stop behavior needed for the first slice
...
- keep the completed branch-state hardening baseline mandatory before any bounded branch creation/switch execution is considered
- keep generalized Git workflow orchestration explicitly out of scope for this model
- keep the still-unresolved branch-target input and first-slice create-vs-switch scope questions explicit where they are not yet safely decided
...
- Do not resolve the open branch-target-input or first-slice create-vs-switch scope questions by assumption; model them as explicit decision points where necessary.
