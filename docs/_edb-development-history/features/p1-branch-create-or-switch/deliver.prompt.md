...
Deliver focus:
- lock the first bounded implementation contract for actual branch creation/switch execution after the completed executor-side `P1 / Minor Change` branch-state hardening step
- reflect the chosen analysis outcome:
  - keep the first branch-mutation slice on the existing `P1` executor continuity path outside the Entry
...
- preserve the completed `P1` contract-emission baseline, the completed first executor-side `P1` minimal write baseline, the completed branch-state hardening baseline, explicit stop behavior, and the explicit exclusion of generalized Git workflow orchestration
- make the first branch-mutation implementation slice explicit enough to authorize later code work without broadening into existing-branch switching, target-resolution redesign, or wider Git workflow automation
...
- Do not broaden scope into switching to an already-existing branch, voluntary mutation from an already-safe non-`main` branch, target-resolution redesign, template-aware generation, `P2`/`P3`/`P4`/`P5` behavior, PR-helper execution, UI/Vite work, or generalized Git workflow orchestration.
